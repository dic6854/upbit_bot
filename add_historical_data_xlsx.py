...
if __name__ == "__main__":
    ...
    for i in range(len(krw_tickers)):
        ...
        file_name = f"mydata/{ticker}.xlsx"
        if os.path.exists(file_name):
            old_df = pd.read_excel(io=file_name, sheet_name=sheet_name, engine='openpyxl', index_col=0, parse_dates=True)
        else:
            old_df = pd.DataFrame()

        file_name = f"data/{ticker}.xlsx"
        add_excel(ticker, interval, start_kst, end_kst, file_name, sheet_name, old_df)
