...
def save_csv(ticker, interval, start_kst, end_kst, file_name):
...
def add_excel(ticker, interval, start_kst, end_kst, file_name, sheet_name, old_df):
    start_kst = set_datetime(start_kst)
    if start_kst == -1:
        return None
    end_kst = set_datetime(end_kst)
    ...
    new_df = fetch_ohlcv_5min(ticker, interval=interval, count=total_minute5, to_kst=end_kst)
...
if __name__ == "__main__":
    ticker = "KRW-BTC"
    interval="minute5"
    start_kst = "2025-03-03 18:21:00"
    end_kst   = "2025-03-06 14:01:00"
    ...
    for i in range(len(krw_tickers)):
        ...
        add_excel(ticker, interval, start_kst, end_kst, file_name, sheet_name)
        save_excel(ticker=ticker, interval=interval, start_kst=start_kst, end_kst=end_kst, file_name=fn, sheet_name=interval, fmode=True)
