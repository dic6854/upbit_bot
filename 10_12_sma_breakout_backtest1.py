...
coin_holdings = {ticker: 0 for ticker in tickers} # 각 코인별 보유량 (개수)
daily_total_assets = {} # 날짜별 총 자산 기록

print(f"--- 백테스트 시작: 거래대금 상위 20개 코인 (백테스트 기간 : {START_DATE_STR} ~ {END_DATE_STR}) ---")
...
while current_backtest_date <= end_date:
    current_date_str = current_backtest_date.strftime("%Y-%m-%d %H:%M:%S")
    print(f"\n===== 날짜: {current_date_str} =====")
    ...
    for ticker in tickers:
        try:
            ...
            ohlcv_today = pyupbit.get_ohlcv(ticker, interval="day", to=current_backtest_date, count=1)

            if ohlcv_today is not None and not ohlcv_today.empty:
                current_prices_for_evaluation[ticker] = ohlcv_today['close'].iloc[-1]
            time.sleep(API_CALL_DELAY) # API 호출 간 딜레이
        except Exception:
            current_prices_for_evaluation[ticker] = 0 # 데이터 없으면 0으로 간주

    day_start_coin_value = sum(coin_holdings[ticker] * current_prices_for_evaluation.get(ticker, 0)
                               for ticker in tickers if coin_holdings[ticker] > 0)
    current_day_total_asset = total_cash + day_start_coin_value
    print(f"  > 시작 자산: {current_day_total_asset:,.0f}원 (현금: {total_cash:,.0f}원, 코인자산: {day_start_coin_value:,.0f}원)")
    ...
    total_coin_value_at_eod = sum(coin_holdings[ticker] * current_prices_for_evaluation.get(ticker, 0) for ticker in tickers)
    daily_total_assets[current_backtest_date.strftime("%Y-%m-%d %H:%M:%S")] = total_cash + total_coin_value_at_eod
