...
def add_excel(ticker, m_unit, start_kst, end_kst):
    ...
    interval = f"minute{m_unit}"
    file_path_in = f"data/{ticker}.xlsx"
    file_path_out = f"data/{ticker}.xlsx"
    sheet_name = interval
    ...
    # 빈 데이터프레임 처리
    if old_df.empty:
        print(f"[{file_path_in} 파일의 {sheet_name} 쉬트]: 기존 데이터가 없음. 전체 기간 데이터를 가져옵니다.")
        ...
        if end_kst1 > old_end:
            print(f"{sheet_name}: 뒷부분 데이터 가져오기 ({old_end} ~ {end_kst1})")
    ...
    print(f"[{ct} / {tt}] - [{file_path_in} 파일의 {sheet_name} 쉬트]: df를 엑셀 파일로 저장 시작")
    ...
    if os.path.exists(file_path_out):
        ...
        # 파일이 존재하지 않는 경우
        with pd.ExcelWriter(path=file_path_out, engine='openpyxl', mode='w') as writer:
            df.to_excel(writer, sheet_name=sheet_name, index=True)

    print(f"[{ct} / {tt}] - [{file_path_in} 파일의 {sheet_name} 쉬트]: df를 엑셀 파일로 저장 완료")
